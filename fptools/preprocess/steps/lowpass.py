...
class LowpassDff(PreprocessorStep):
    """A `Preprocessor` that calculates signal dF/F, using a lowpass filter to estimate F0."""
    ...
    def __init__(self, signals: SignalList, frequency: float = 0.01):
        ...
        self.lowpass_signals: dict[str, Signal] = {}
    ...
    def __call__(self, session: Session) -> Session:
        ...
        for signame in self._resolve_signal_names(session, self.signals):
            ...
            lowpass_sig = sig.copy()
            lowpass_sig.signal = lowpass_filter(sig.signal, sig.fs, self.frequency)
            self.lowpass_signals[signame] = lowpass_sig

            sig.signal = (((sig - lowpass_sig) / lowpass_sig) * 100).signal
            sig.units = "ΔF/F"
        return session

    def plot(self, session: Session, ax: Axes):
        """Plot the effects of this preprocessing step. Will show the lowpass filtered signal and the corrected signal.
        ...
        for i, signame in enumerate(signals):
            sig = session.signals[signame]
            ax.plot(sig.time, sig.signal, label=f"lowpass filtered {sig.name}", c=palette[i], linestyle="--")
            ax.plot(sig.time, sig.signal, label=f"corrected {sig.name}", c=palette[i], linestyle="-")
        ax.set_title("Lowpass DFF")
        ax.legend()
